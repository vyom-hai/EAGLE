...
import math
import time  # [PROFILING] Added for performance measurement
...
# [PROFILING] Simple profiling utilities
ENABLE_PROFILING = True  # Set to True to enable detailed timing
_profile_times = {}

def profile_start(name):
    if not ENABLE_PROFILING:
        return
    if name not in _profile_times:
        _profile_times[name] = []
    _profile_times[name].append(time.perf_counter())

def profile_end(name):
    if not ENABLE_PROFILING:
        return
    if name in _profile_times and _profile_times[name]:
        elapsed = time.perf_counter() - _profile_times[name][-1]
        _profile_times[name][-1] = elapsed

def profile_report():
    if not ENABLE_PROFILING or not _profile_times:
        return
    print("\n" + "="*70)
    print("topK_genrate PROFILING")
    print("="*70)
    for name in sorted(_profile_times.keys()):
        times = [t for t in _profile_times[name] if isinstance(t, float)]
        if times:
            mean_ms = sum(times) / len(times) * 1000
            print(f"  {name:<50} {mean_ms:>10.3f} ms")
...
class Model(nn.Module):
    ...
    def topK_genrate(self, hidden_states, input_ids, head, logits_processor):
        profile_start('topK_genrate_total')  # [PROFILING]
        ...
        # Pre-allocate final tree mask for incremental construction (optimization)
        profile_start('mask_prealloc')  # [PROFILING]
        final_tree_mask = torch.eye(total_tokens + 1, dtype=torch.bool, device=self.embed_tokens.weight.device)
        final_tree_mask[:, 0] = True  # All tokens can attend to root
        profile_end('mask_prealloc')  # [PROFILING]
        token_counter = 1  # Track position in final mask (0 is root)
        ...
        for i in range(depth):
            ...
            profile_start('tree_mask_concat')  # [PROFILING]
            tree_mask = torch.cat((tree_mask[:, :, out_ids], self.tree_mask_init), dim=3)
            profile_end('tree_mask_concat')  # [PROFILING]
        ...
        # Keep mask_index_list for compatibility with later code
        profile_start('tolist_call')  # [PROFILING]
        mask_index_list = mask_index.tolist()
        profile_end('tolist_call')  # [PROFILING]
        ...
        profile_start('retrieve_indices')  # [PROFILING]
        ...
        for i in range(total_tokens + 1):
            if i not in noleaf_index:
                ...
                rid += 1
        profile_end('retrieve_indices')  # [PROFILING]
        ...
        profile_end('topK_genrate_total')  # [PROFILING]
        profile_report()  # [PROFILING] Print profiling report
